...
public var mBZoomIn : MyButton;
public var mBZoomOut : MyButton;
public var mBRotateLeft : MyButton;
public var mBRotateRight : MyButton;
...
// -------------------------- Public Methods --------------------------
function Start () {
	currentCameraDistance = minZoomDistance + ((maxZoomDistance - minZoomDistance) / 10.0f);
...
private function UpdateRotation() {
	...
	if (mBRotateLeft.mIsDown){
		deltaAngleH = -1.0f;
	}
	if (mBRotateRight.mIsDown){
		deltaAngleH = 1.0f;
	}
	...
	/*var newX : float  = Mathf.Min(80.0f, Mathf.Max(5.0f, transform.localEulerAngles.x + deltaAngleV * Time.deltaTime * rotationSpeed));
	if (newX < 30.0f){
		newX = 30.0f;
	}*/
	var newX : float = 30.0f;
	transform.localEulerAngles.x = newX;
}
...
public function doZoomPintch(){
    ...
    if (Input.touchCount == 2)
    {
        ...
        var deltaZoom : float = deltaMagnitudeDiff / 2;
